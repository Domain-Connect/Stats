...
```
.
├── dashboard/              # Statistics dashboard output directory
...
5. Generate `dashboard/stats.json` with complete dataset
...
3. Update `dashboard/index.html` to display the new data
4. Update `dashboard/_claude_prompt.txt` to reflect the new requirements
5. Update this `CLAUDE.md` to document the change

**Important:** `dashboard/_claude_prompt.txt` and `CLAUDE.md` must always be kept in sync with the actual implementation. Any change to dashboard features, chart types, data sources, script arguments, or interactive behavior must be reflected in both files.
...
2. Open `dashboard/index.html` in browser to verify display
...
- `dashboard/index.html`: Layout and JavaScript for visualization
- `dashboard/styles.css`: Styling and responsive design
- `.github/workflows/update-stats.yml`: CI/CD automation

### What NOT to Edit Manually
- `dashboard/stats.json`: Auto-generated by update_stats.py
